...
A competency development platform: admin-registered users take an annual **Competency Assessment**, the system computes the gaps between their assessed levels and the target levels their role requires, and those gaps become a personal **Growth Plan** — tracked over a **3-year cycle**.

It started as a focused slice of a much larger spec — **role-based access with separation of duties** and the **assessment → gap → plan cycle engine** — and grew into a full L&D platform (see [Features](#features)).
...
| Role | What they do |
|---|---|
| **Employee** | The learner: take own assessment, view own gaps/plan, log training. |
| **Supervisor** | Validate their team's assessments, endorse plans, verify training. |
| **HR / L&D Admin** | Org-wide: final Growth Plan approval, org reports, the content library, the cycle scheduler, user management, the audit log. |
| **Super Admin** | Everything, plus the configurable permission matrix. |

Only **employees** are learners (their own assessment, gaps, plan, and training). Supervisors, HR, and super-admins are management roles — they don't have their own development plan. **Separation of duties:** no one can validate their own assessment or approve their own plan. Capabilities are a **configurable matrix** — a super-admin tunes which role holds which capability, enforced in the UI, the routes, *and* the database (RLS).

## How it works
1. **Baseline (Year 1).** An employee self-rates each competency their role requires. A supervisor validates the ratings. The system locks the role's targets into the cycle, computes the gaps (`target − assessed`), and generates the Growth Plan.
...
3. **Annual re-assessment (Years 2–3).** The employee retakes the assessment; the system diffs the new levels against the locked targets *and* the previous year, updates the same plan in place, and writes a progress snapshot (the 3-year trend).
...
- **Competency Assessment → gap → Growth Plan engine** — annual self-assessment, a pure gap/diff/readiness classifier, and a 3-year development cycle.
...
- **Cycle scheduler** — open development cycles in bulk, set assessment due dates, and advance every active cycle a year (or close it) at once.
- **In-app notifications** — a database-trigger-driven feed for assessment validation, plan endorsement and approval, and cycle open / advance / close.
...
- The multi-table **validate → roll-up → gap → plan → snapshot → audit** transaction runs in a **server route holding the secret key**, after re-checking role + scope + SoD *in code*. RLS is defense-in-depth; the route is the real boundary (the shape of a payment-confirm endpoint).
...
# 1. Run the SQL files in supabase/migrations/ in order (0001 → 0010) in the
#    Supabase SQL editor — schema + scope-aware RLS + the DICT NICS seed, then
#    each later phase (deep assessment, permissions, notifications, org units, capability RLS).
...
- **E2E (Playwright):** the employee → supervisor → HR flow plus RBAC, reporting, the cycle scheduler, bulk import, org units, notifications, and integrations (13 specs).

## Scope
**Built:** the full platform described in [Features](#features) — on seeded demo data, covered by ~130 unit assertions and 13 end-to-end specs.
...
## Credits
Competency library, proficiency levels, and target language adapted from the **DICT National ICT Competency Standards (NICS)**, which are freely reusable. The TESDA Self-Assessment Guides informed the "Can I…?" assessment format (the items here are our own wording). Caliber's annual-assessment → development-plan model follows the Philippine public sector's **Training Needs Analysis → Individual Learning & Development Plan (TNA → ILDP)** approach.
